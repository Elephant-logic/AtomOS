...
function scanPython(code) { const source=String(code||''),errors=[]; for(const [pattern,message] of BLOCKED_PATTERNS) if(pattern.test(source)) errors.push(message); const dependencies=new Set(); for(const match of source.matchAll(/^\s*(?:from|import)\s+([A-Za-z0-9_.]+)/gm)) dependencies.add(match[1].split('.')[0]); const functions=[...source.matchAll(/^def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/gm)].map(x=>x[1]); const classes=[...source.matchAll(/^class\s+([A-Za-z_][A-Za-z0-9_]*)\b/gm)].map(x=>x[1]); return {errors,dependencies:[...dependencies].sort(),functions,classes,lineCount:source.split('\n').length}; }
