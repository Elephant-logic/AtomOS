...
function scanPreview(html) {
  ...
  const errors = [];
  for (const [pattern, message] of PREVIEW_BLOCKED) if (pattern.test(source)) errors.push(message);
  ...
  if (mentionsStrictDate && !usesDateInput && !normalizesSeparators) errors.push('date input must use type=date or normalize slash/dot separators before validation');
...
async function smokeTestPython(code) {
  return withTempProgram(code, async ({dir}) => {
    ...
    const result=await runProcess(process.env.PYTHON_BIN||'python3',['-I','-c',harness],{cwd:dir,env:{PATH:process.env.PATH||'',HOME:dir,TMPDIR:dir,PYTHONNOUSERSITE:'1',DISPLAY:''},timeoutMs:5000});
    return {ok:result.ok,error:result.stderr.trim(),stdout:result.stdout.trim().slice(0,2000),timedOut:result.timedOut,mode:'headless import smoke test with Tkinter stubs',limitations:'The verification host does not include Tkinter or a display. AtomOS stubs GUI modules only while importing the program, then displays the separate functional browser preview.'};
  });
}
