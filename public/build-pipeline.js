(() => {
    ...
    request = async function pipelineRequest(editing) {
      ...
      try {
        for (const item of before) {
          ...
          if (result === false || context.cancelled) return null;
        ...
        context.currentApp = typeof currentApp === 'undefined' ? window.currentApp : currentApp;
        for (const item of after) await item.handler(context);
        return result;
      } catch (error) {
        context.error = error;
        for (const item of failed) {
          try { await item.handler(context); } catch (nested) { console.warn('AtomOS middleware error', item.name, nested); }
        }
        throw error;
      } finally {
  ...
  const api = { useBefore, useAfter, useError, install, before, after, failed };
  window.AtomOSBuildPipeline = api;
  let attempts = 0;
