(() => {
  ...
  function connectors(source, name, inputs = []) {
    const required = uniq(inputs.map(x => `value:${x}`).concat([...source.matchAll(/\b(?:fetch|requests\.(?:get|post|put|delete))\s*\(\s*['"]([^'"]+)/g)].map(x => `api:${x[1]}`))).slice(0, 20);
    const provided = uniq([...source.matchAll(/\b(?:dispatch|emit)\s*\(\s*['"]([^'"]+)/g)].map(x => `event:${x[1]}`).concat(name ? [`function:${slug(name)}`] : [])).slice(0, 20);
  ...
  function translatedPart({ name, kind, file, source, language, confidence = .8, extra = {} }) {
    ...
    return {
      ...
      atomos:{ languageVersion:'0.1', kind, inputs:inputs.map(input => ({ name:input, type:'unknown' })), outputs:[{ name:'result', type:'unknown' }], operations, translationStatus:operations.some(x => x.op === 'external_implementation') ? 'external' : 'partial', implementations:{ [language]:{ source:String(source).slice(0,12000) } } },
      ...extra
    };
  }
  ...
  function analyse(file) {
    ...
    if (/javascript|typescript|react|vue|svelte/.test(language)) {
      for (const match of source.matchAll(/(?:export\s+(?:default\s+)?)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*\([^)]*\)[\s\S]{0,1800}?(?=\n(?:export\s+)?(?:async\s+)?function\s+|$)/g)) parts.push(translatedPart({ name:match[1], kind:/^[A-Z]/.test(match[1])?'ui.component':'logic.function', file:file.name, source:match[0], language, confidence:.9 }));
      for (const match of source.matchAll(/\b(?:app|router)\.(get|post|put|patch|delete)\s*\(\s*['"]([^'"]+)/g)) parts.push(translatedPart({ name:`${match[1].toUpperCase()} ${match[2]}`, kind:'api.route', file:file.name, source:match[0], language, confidence:.95, extra:{ method:match[1].toUpperCase(), path:match[2] } }));
    }
    if (language === 'python') {
    ...
    if (language === 'html') for (const match of source.matchAll(/<(form|button|input|nav|section|dialog|canvas)\b[^>]*(?:id=['"]([^'"]+)['"])?[^>]*>/gi)) parts.push(translatedPart({ name:match[2] || match[1], kind:`ui.${match[1].toLowerCase()}`, file:file.name, source:match[0], language, confidence:.82 }));
  ...
  function build(files) {
    ...
    return { atomosLanguageVersion:'0.1', createdAt:new Date().toISOString(), summary:{ files:files.length, atoms:atoms.length, molecules:molecules.length, translated:atoms.filter(x => x.atomos.translationStatus !== 'external').length }, files:analysed.map(({parts,...file})=>file), atoms, molecules, edges };
  }
  ...
  function saveTranslation(value) {
    ...
    const library=JSON.parse(localStorage.getItem(LANGUAGE_KEY)||'[]'); library.push(...value.atoms.map(atom=>({ id:`${atom.id}-${Date.now()}`, name:atom.name, kind:atom.kind, connectors:atom.connectors, atomos:atom.atomos, sourceFile:atom.sourceFile }))); localStorage.setItem(LANGUAGE_KEY,JSON.stringify(library.slice(-500)));
  }
  ...
  function mount() {
    ...
    const section=document.createElement('section'); section.id='codeRefinery'; section.innerHTML=`<hr style="border:0;border-top:1px solid var(--line);margin:14px 0"><h2>Code translator</h2><p class="muted">Upload Python, JavaScript or project files. AtomOS translates functions, routes, connectors and supported behaviour into AtomOS Language while retaining the original runtime implementation.</p><input id="refineryFiles" type="file" multiple style="width:100%"><div class="row"><button id="analyseCode" class="primary">Translate code</button><button id="saveRefinery" disabled>Save to language library</button><button id="downloadRefinery" disabled>Download translation</button></div><div id="refineryOutput" class="muted" style="margin-top:9px">No code translated yet.</div>`; left.appendChild(section);
    const input=document.getElementById('refineryFiles'); input.accept=[...EXT].map(x=>`.${x}`).join(',');
    document.getElementById('analyseCode').onclick=async()=>{ const button=document.getElementById('analyseCode'); button.disabled=true; button.textContent='Translating…'; try { result=build(await readFiles(input.files)); render(document.getElementById('refineryOutput'),result); document.getElementById('saveRefinery').disabled=false; document.getElementById('downloadRefinery').disabled=false; if(typeof log==='function')log(`Translated ${result.summary.atoms} parts into AtomOS Language.`,'ok'); } catch(error) { document.getElementById('refineryOutput').textContent=error.message; if(typeof log==='function')log(error.message,'bad'); } finally { button.disabled=false; button.textContent='Translate code'; } };
