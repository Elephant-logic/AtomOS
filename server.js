...
async function knowledgeRoute(req, res, url) {
  if (!url.pathname.startsWith('/api/knowledge')) return false;
  try {
    if (req.method === 'GET' && url.pathname === '/api/knowledge/stats') return send(res, 200, await knowledge.stats()), true;
    if (req.method === 'GET' && url.pathname === '/api/knowledge/search') return send(res, 200, await knowledge.search(url.searchParams.get('q') || '', Number(url.searchParams.get('limit') || 30))), true;
    ...
    if (req.method === 'POST' && url.pathname === '/api/knowledge/import') return send(res, 200, await knowledge.importAtoms(JSON.parse(await readBody(req, 5_000_000) || '{}'))), true;
    if (req.method === 'POST' && /\/api\/knowledge\/atoms\/[^/]+\/status$/.test(url.pathname)) {
      const id = decodeURIComponent(url.pathname.split('/').slice(-2)[0]);
      const body = JSON.parse(await readBody(req) || '{}'); send(res, 200, await knowledge.setStatus(id, body.status)); return true;
    }
    if (req.method === 'POST' && /\/api\/knowledge\/atoms\/[^/]+\/usage$/.test(url.pathname)) {
      const id = decodeURIComponent(url.pathname.split('/').slice(-2)[0]);
      const body = JSON.parse(await readBody(req) || '{}'); send(res, 200, await knowledge.recordUsage(id, Boolean(body.success))); return true;
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (await knowledgeRoute(req, res, url)) return;
