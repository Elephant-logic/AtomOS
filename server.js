...
const { resolvePrimitives, primitiveInstructions } = require('./src/primitive-resolver');
...
async function resolvePlanPrimitives(prompt, plan) {
  return resolvePrimitives(prompt, plan, {
    knowledge,
...
async function recordPrimitiveUsage(resolution, success) {
  for (const id of [...(resolution?.reused || []), ...(resolution?.created || [])]) {
    try { await knowledge.recordUsage(id, success); } catch {}
  }
}
...
async function compileAndBuild(prompt,currentApp,intent){
  ...
  if(!currentApp){
    ...
    if(builtin){
      ...
      return {app,model:'builtin',responseId:null,mode:'build',capabilities:[],builtin:builtin.id,plan,primitiveResolution:{reused:[],created:[],atoms:[],instructions:[],summary:'known builtin'}};
    }
  }

  const primitiveResolution=await resolvePlanPrimitives(prompt,plan);
  const guarded=`BUILD PROFILE: ${intent.intent}. ${intent.instructions}\n${planInstructions(plan)}\n${primitiveInstructions(primitiveResolution)}\n\nUSER REQUEST:\n${prompt}`;
  let result;
  try {
    result=await buildApp(guarded,currentApp);
  } catch(error) {
    const repair=`${guarded}\n\nREPAIR REQUIREMENT: A previous attempt failed validation with: ${error.message}. Return a corrected complete application assembled from the resolved primitives. Use scene for real-time games and sound for audio. Every rule action target must exactly match a declared state key.`;
    try {
      result=await buildApp(repair,currentApp);
      result.repaired=true;
    } catch (repairError) {
      await recordPrimitiveUsage(primitiveResolution,false);
      throw repairError;
    }
  }
  await recordPrimitiveUsage(primitiveResolution,true);
  return {...result,plan,primitiveResolution,learnedAtoms:primitiveResolution.created};
}
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url,'http://localhost');if(await knowledgeRoute(req,res,url))return;
  if(req.method==='GET'&&url.pathname==='/api/status')return send(res,200,{ready:Boolean(process.env.OPENAI_API_KEY),model:process.env.OPENAI_MODEL||'gpt-5-mini',capabilityTypes:['interval','storage','startup','keyboard'],actionTypes:ACTION_TYPES,componentTypes:APP_SCHEMA.properties.components.items.properties.type.enum,screens:true,boards:true,groups:true,repeats:true,scenes:true,audio:true,capabilityCompiler:true,primitiveResolver:true,conditionalActions:true,conditionalComponents:true,internalEvents:true,listState:true,intentRouter:true,codeBuilder:{languages:['python'],execution:false,verification:['blocked-pattern scan','python syntax compile','repair loop']}});
  ...
  if(req.method==='POST'&&url.pathname==='/api/primitive-resolve'){try{const body=JSON.parse(await readBody(req)||'{}');const prompt=String(body.prompt||'').trim();if(prompt.length<3||prompt.length>12000)return send(res,400,{error:'Prompt must be between 3 and 12000 characters'});const plan=compileCapabilityPlan(prompt);return send(res,200,{plan,resolution:await resolvePlanPrimitives(prompt,plan)});}catch(error){return send(res,500,{error:error.message||'Primitive resolution failed'});}}
...
module.exports={normalizeApplication,validateReferences,buildApp,APP_SCHEMA,ACTION_TYPES,classifyBuildIntent,applyIntentGuard,compileCapabilityPlan,resolvePlanPrimitives,compileAndBuild,server};
